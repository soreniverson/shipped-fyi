...
import { createSlackIntegrationData } from '@/lib/integrations/slack'
...
export async function GET(request: NextRequest) {
  ...
  // Parse state to get project ID and verify CSRF token
  ...
  try {
    ...
    if (!oauthData.ok) {
      console.error('Slack token exchange failed:', oauthData.error)
      return NextResponse.redirect(
        new URL(
    ...
    const integrationData = createSlackIntegrationData(oauthData)
    ...
    const { data: existingIntegration } = await supabase
      ...
      .eq('type', 'slack')
      .contains('config', { team_id: integrationData.config.team_id })
      .single()
